...
1 one `n` symbol long substring,
2 `n - 1` symbol long substrings,
...,
`n` single symbol long substrings
Sum of arithmetic progression + one for an empty substring.

e) Subsequences.
**Answer:**  `Sum(i = 0, n)C(n, i)`,

where `C(n, i)` is number of `i` unique combinations of `n` elements.

## Exercise 3.3.4
Most languages are *case sensitive* so keywords can be written only one way and
the regular expressions describing their lexemes are very simple. However, some
...
f) All strings of *a*'s and *b*'s with an even number of *a*`s and an odd
number of *b*'s.

**Answer:**
```
even -> (aa|bb)*((ab|ba)(aa|bb)*(ab|ba)(aa|bb)*)*
answer -> even b even
```
...
**Answer:**
```
move -> capturing | non_capturing | castling | promotion
non_capturing -> piece '-' square
capture -> piece x piece
castling -> 'O-O' | 'O-O-O'
promotion -> [capturing non_capturing] '(' [QRBN] ')'
piece -> [QKRBNP] | [QK][RBN] | [QK][RBN]P
square -> [QK][RBN]?[1-8]
```
